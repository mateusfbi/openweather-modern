    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (this._translate_condition && !this._providerTranslations)
                comment = getWeatherCondition(json.weather[0].id, this._);
            ...
            this._weatherInfo.text = weatherInfoC + ((weatherInfoC && weatherInfoT) ? this._(", ") : "") + weatherInfoT;

            this._currentWeatherSummary.text = comment + this._(", ") + temperature;
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            for (var i = 0; i < 4; i++) {
                ...
                if (this._translate_condition && !this._providerTranslations)
                    comment = getWeatherCondition(forecastDataToday.weather[0].id, this._);
        ...
        try {
            ...
            for (let i = 0; i < this._forecastDays; i++) {
                ...
                for (let j = 0; j < 8; j++) {
                    ...
                    if (this._translate_condition && !this._providerTranslations)
                        comment = getWeatherCondition(forecastData[j].weather[0].id, this._);
