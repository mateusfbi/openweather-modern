...
let OpenWeatherMenuButton = GObject.registerClass(
    class OpenWeatherMenuButton extends PanelMenu.Button {
        ...
        useOpenWeatherMap() {
            // Store translation function for use in OpenWeatherMap functions
            this._ = _;
            ...
            this.loadJsonAsync = (url, params) => OpenWeatherMap.loadJsonAsync.call(this, url, params, this._extension.metadata);
        ...
        loadConfigInterface() {
            this._settingsInterface = new Gio.Settings({ schema: 'org.gnome.desktop.interface' });
        ...
        getWeatherIcon(iconname) {
            ...
            if (this._getUseSysIcons && Gtk.IconTheme.get_default().has_icon(iconname)) {
                ...
                return Gio.icon_new_for_string(this._extension.path + "/media/status/" + iconname + ".svg");
            }
        }
        ...
        rebuildCurrentWeatherUi() {
            ...
            rb_captions.add_child(new St.Label({
                text: _('Feels Like:')
            }));
            ...
            this._todaysBox = new St.BoxLayout({
                x_expand: true,
                x_align: this._center_forecast ? Clutter.ActorAlign.END : Clutter.ActorAlign.START,
            ...
            for (let i = 0; i < 4; i++) {
                ...
                if (this._comment_in_forecast)
                    fb.add_child(todaysForecast.Summary);
        ...
        rebuildFutureWeatherUi(cnt) {
            ...
            for (let i = 0; i < cnt; i++) {
                ...
                for (let j = 0; j < 8; j++) {
                    ...
                    by.add_child(bib);
                    if (this._comment_in_forecast)
                        by.add_child(forecastWeather[j].Summary);
...
export default class OpenWeatherExtension extends Extension {
    enable() {
        this._indicator = new OpenWeatherMenuButton(this);
