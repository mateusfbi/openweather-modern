...
export const AboutPage = GObject.registerClass(
    class OpenWeather_AboutPage extends Adw.PreferencesPage {
        _init(extension) {
            ...
            let releaseVersion = (extension.metadata.version) ? extension.metadata.version : _("unknown");
            let gitVersion = (extension.metadata['git-version']) ? extension.metadata['git-version'] : null;
            ...
            let maintainerAbout = new Gtk.Label({
                label: _("Maintained by: %s").format("Mateus Brandão"),
                hexpand: false,
                vexpand: false
            });

            let pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(extension.path + '/media/donate-icon.png', -1, 50, true);
